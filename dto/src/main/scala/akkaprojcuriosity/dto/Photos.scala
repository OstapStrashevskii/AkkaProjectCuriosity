...
//todo not typed yet
final case class Photos (photos: Seq[Photo])
...
final case class Photo (id: Int, camera: Camera, img_src: String, earth_date: String, rover: Rover)

object Photo {
  implicit lazy val photoFormat: OFormat[Photo] = Json.format[Photo]
}

final case class Camera (id: Int, name: String, rover_id: Int, full_name: String)
...
//todo the same
final case class Rover(id: Int, name: String, launch_date: String, status: String, max_sol: Int, max_date: String, total_photos: Int)
